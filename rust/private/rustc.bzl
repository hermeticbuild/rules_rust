    ...
    if ("link" in emit and crate_info.type not in ["rlib", "lib"]) or add_flags_for_binary:
        ...
        if toolchain.target_arch not in ("wasm32", "wasm64"):
            if output_dir:
                ...
                rpaths = _compute_rpaths(
                    toolchain,
                    output_dir,
                    dep_info,
                    use_pic,
                    link_std_dylib,
                    output_file = crate_info.output,
                    workspace_name = ctx.workspace_name,
                    dynamic_runtime_libs = rpath_runtime_libs,
                )
            else:
                rpaths = depset()
...
def rustc_compile(
    ...
    if feature_configuration and cc_helper.should_create_per_object_debug_info(
        ...
        if rust_toolchain._skip_fission_for_rust:
            use_split_debuginfo = False
        elif rust_toolchain.channel == "nightly":
            ...
            fail(
                ...
                "(current toolchain channel is \"{}\"). ".format(rust_toolchain.channel) +
    ...
    if hasattr(attr, "link_std_dylib"):
        link_std_dylib = rust_toolchain._link_std_dylib or attr.link_std_dylib
    else:
        link_std_dylib = rust_toolchain._link_std_dylib
    ...
    runfiles = ctx.runfiles(
        files = getattr(files, "data", []) +
                ...
                ([rust_toolchain.rust_std_dylib] if link_std_dylib and rust_toolchain.rust_std_dylib else []),
    )
    transitive_runfiles = []
    ...
    if crate_info.is_test or crate_info.type in ["bin", "cdylib", "dylib", "staticlib"]:
        cc_providing_deps = getattr(attr, "deps", []) + getattr(attr, "link_deps", []) + ([crate_attr] if crate_attr else [])
        dep_dylib_files = [
    ...
    if rust_toolchain.target_os == "windows" and (crate_info.type == "bin" or crate_info.is_test):
        win_dylibs = list(dep_dylib_files)
        if link_std_dylib:
            if rust_toolchain.rust_std_dylib:
                win_dylibs.append(rust_toolchain.rust_std_dylib)
...
def _compute_rpaths(toolchain, output_dir, dep_info, use_pic, link_std_dylib, output_file = None, workspace_name = "", dynamic_runtime_libs = None):
