...
def construct_arguments(
    ...
    Args:
        ...
        runtime_libs (depset[File], optional): Runtime libraries from the C++ toolchain
            selected for `crate_info.type`.
        object_output (File, optional): The path for an `obj` emit.
    ...
    for kind in emit:
        ...
        elif kind == "obj" and object_output != None:
            rustc_flags.add(object_output, format = "--emit=obj=%s")
        else:
...
def rustc_compile(
    ...
    if not _supports_distributed_thin_lto(ctx, rust_toolchain, crate_info):
        extra_unsupported_features.append("thin_lto")
    ...
    # When build_metadata is set, a separate -Zno-codegen Rustc action emits
    # metadata. This Rustc action emits crate_info.output unless cc_common.link
    # does, and emits output_o when output_o is declared.
    emit = []
    ...
    # By default the Rustc action produces `crate_info.output`. cc_common.link
    # consumes `output_o` and produces `crate_info.output` in a separate action.
