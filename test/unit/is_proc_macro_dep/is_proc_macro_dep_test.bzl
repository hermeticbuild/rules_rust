...
def _find_rust_compile_action(actions, target_name):
    target_fragment = ":%s " % target_name

    for action in actions:
        action_str = str(action)
        if action_str.startswith("action 'Compiling Rust ") and target_fragment in action_str:
            return action

    fail("No Rust compile action found for %s. Actions:\n%s" % (
        target_name,
...
def _is_proc_macro_dep_is_false_for_proc_macro(ctx):
    ...
    proc_macro_dep_action = _find_rust_compile_action(tut[DepActionsInfo].actions, "proc_macro_crate")
...
def _is_proc_macro_dep_is_false_for_top_level_library(ctx):
    env = analysistest.begin(ctx)
    tut = analysistest.target_under_test(env)
    top_level_library_action = _find_rust_compile_action(tut[DepActionsInfo].actions, "top_level_library")
...
def _is_proc_macro_dep_is_true_for_proc_macro_dep(ctx):
    env = analysistest.begin(ctx)
    tut = analysistest.target_under_test(env)
    proc_macro_dep_action = _find_rust_compile_action(tut[DepActionsInfo].actions, "proc_macro_dep")
